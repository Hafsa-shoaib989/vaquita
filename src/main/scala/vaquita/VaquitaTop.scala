...
class VaquitaTop extends Module {
    ...
    val opcode = RegNext(io.instr(6,0))
    dontTouch(opcode)
    val func3 = RegNext(io.instr(14,12))
    dontTouch(func3)
...
// -----------------decode stage ---------------------------------
    ...
    EX.ex_fp_scalarM_alu_op_in := DE.de_io.fp_scalarM_alu_op_out

    // ************forwording unit***********************
    DE.de_io.vl_rs1_in := RegNext(EX.vl_rs1_out.asUInt)
    EX.ex_lmul_in := DE.de_io.lmul_out
    val a = RegNext(io.instr(14,12))
    val b = RegNext(io.instr(6,0))
    io.vl_rs1_out := Mux((a === "b111".U) & (b === "b1010111".U), EX.vl_rs1_out.asUInt, DE.de_io.de_reg_rd.asUInt) //func3, opcode
    EX.hazard_rs1 := RegNext(io.hazard_rs1_data_in)
